...
static const uint16 kDefaultKbdControls[kKeys_Total] = {
  ...
  // VolumeUp, VolumeDown, ToggleChase
  N, N, _(SDLK_4),
  // RotateLeft, RotateRight, ZoomIn, ZoomOut
  _(SDLK_q), _(SDLK_e), _(SDLK_EQUALS), _(SDLK_MINUS),
};
#undef _
#undef A
#undef C
...
static const KeyNameId kKeyNameId[] = {
  ...
  S(Pause), S(PauseDimmed), S(Turbo), S(ReplayTurbo), S(WindowBigger), S(WindowSmaller), S(VolumeUp), S(VolumeDown), S(DisplayPerf), S(ToggleRenderer), S(ToggleVoxel), S(ToggleChase),
  S(RotateLeft), S(RotateRight), S(ZoomIn), S(ZoomOut),
};
#undef S
#undef M
...
static bool HandleIniConfig(int section, const char *key, char *value) {
  if (section == 0) {
    ...
    if (StringEqualsNoCase(key, "WindowSize")) {
      ...
      return true;
    } else if (StringEqualsNoCase(key, "VoxelCamera")) {
      g_config.voxel_camera = (uint8)IntMax(0, IntMin(3, atoi(value)));
      return true;
    } else if (StringEqualsNoCase(key, "VoxelChaseCam")) {
      // Legacy bool form of VoxelCamera.
      bool chase = false;
      if (!ParseBool(value, &chase))
        return false;
      g_config.voxel_camera = chase ? 1 : 0;
      return true;
